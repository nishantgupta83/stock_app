...
| [`2026-06-12_e2e-review.md`](2026-06-12_e2e-review.md) | 2026-06-12 | **Criticals + H3/H4-writers/H6 DONE; progress log inside** | End-to-end review + session progress log. Shipped + validated: C1 (calibration ratchet), C2 (maturity-gate scope + PF-lag), C3 (swallowed errors), H3, H4 DB-writers, H6 (risk pagination). Remaining work → `2026-06-12_remaining-prs.md`. |
| [`2026-06-12_remaining-prs.md`](2026-06-12_remaining-prs.md) | 2026-06-12 | **Forward PR queue** | Prioritized one-behavior-per-PR queue for the open items: H1 effective-n, H2 after-hours anchor, H8 daily-budget batch, H5 retry lane-scope, H4b payoff-first display redesign, H7 watchdog; M1–M8 (M2/M3 before the 2026-07-06 funnel re-run); Lows. Each with scope, approach, measurement gate, risk, and order. |
...
| [`2026-09-07_options-research.md`](2026-09-07_options-research.md) | 2026-09-07 | **Research + design (deferred)** | Options primer, seller-side evidence (Cboe PUT/BXM, VRP literature, retail-loss studies, tail events), free/paper infrastructure table (Alpaca paper is the only $0 headless stack with spreads + greeks), and the `opt_pcs_v1` isolated experiment design — pre-registration draft in `docs/experiments/2026-09-08-preregistration-opt-pcs-v1.md`. |
| [`2026-08-02_schema-reconciliation.md`](2026-08-02_schema-reconciliation.md) | 2026-08-02 | **Baseline — Step-2 precondition** | Live DB reconciliation: `sql/` (42 files) applied out-of-band is the source of truth; the `supabase/migrations/` CLI ledger is stale (9 files, last 2026-05-22) — NEVER `db push`. `sql/0039` candidate ledger is LIVE + populated (217 rows) despite its "draft" header; 2.a works, 2.b/orchestrator not (gate_decision NULL). Step 2 shrinks accordingly. |
