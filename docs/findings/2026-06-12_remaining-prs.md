...
| **M6** ✅ 2/3 DONE | (1) backtester partial-insert mislabelled audit; (2) flows 13F truncation fabricated events; (3) market_scanner NULL dupes | ✅ (1) per-chunk pairing `_insert_signals_chunked` (TDD); ✅ (2) fetch_prior_snapshot scopes to latest filing + paginates (TDD). ⬜ (3) M6.3: recreate the (ticker,observed_at,prior_event_id) unique index with NULLS NOT DISTINCT (PG15+) — migration + dedup-first; duplicate rows = noise not corruption | 2/3 done |
