...
| **M1** ✅ DONE `a1b074a` | backtester upserted live `stock_agent_weights` → thesis/dashboard read it blind | loop-isolation guard: no live write by default (opt-in BACKTEST_WRITE_LIVE_WEIGHTS); existing self-heals (TDD). ⬜ M1-2: separate table + source col | core done |
| **M4** ✅ CORE DONE | L1 ingest agents finish ok on bus-write failure; no event-volume pulse | ✅ event_bus_volume pulsecheck (warns on total ingest collapse, market-hours+holiday-aware, far-below-normal floor; TDD). ⬜ M4-2: per-agent surfacing of upstream failure (EDGAR all-429 → status=partial) — needs the write to signal failure vs empty (same class as deferred C3 read-collapse) | core done |
...
| **M6** ✅ DONE (3/3) | backtester audit mislabel; flows 13F truncation; market_scanner NULL dupes | ✅ (1) per-chunk pairing (TDD); ✅ (2) prior-snapshot scoped+paginated (TDD); ✅ (3) sql/0042 applied live — deduped 86 NULL-prior rows + recreated unique index NULLS NOT DISTINCT (PG17, no FK refs, on_conflict unchanged) | done |
| **M7** ✅ DONE | egress docs/estimator/bootstrap | MOSTLY STALE (re-validated live): egress now **3.27 GB/mo (65%)** not 3.92/78%; --live verdict NOT-worth-blob ALIGNS with CLAUDE.md; pulsecheck already modeled; site_generator pinned 1/day; rule #6 cancel-in-progress accurate (only retry excepted). Fixed: stale READ_MAP 161/day comments→EOD, pure-model blob verdict now references the documented rejection, CLAUDE.md rule #6 wording, READ_MAP notes H6's productive-run-gated risk reads. ⬜ bootstrap-file-vs-provisioned needs a USER cron-job.org console check before any re-run | done (1 user-action) |
