...
def test_production_mature_rule_gets_full_multiplier():
    ...
    cal = {"mature:foo:h7d": {"is_mature": True, "accuracy": 0.92, "n_observations": 35}}
    decision = evaluate_setup(setup, cal=cal, state=_state())
    expected_risk = PORTFOLIO_NAV_BASELINE * RISK_PER_TRADE_PCT * MATURITY_MULTIPLIER["production"]
    assert decision["max_loss_dollars"] == pytest.approx(expected_risk, abs=0.01)


def test_training_tier_gets_half_multiplier():
    setup = _setup(rule_key="training:foo:h7d")
    # Below is_mature but accuracy ≥ 0.70 and n ≥ 30 → training tier
    cal = {"training:foo:h7d": {"is_mature": False, "accuracy": 0.75, "n_observations": 32}}
    decision = evaluate_setup(setup, cal=cal, state=_state())
    expected_risk = PORTFOLIO_NAV_BASELINE * RISK_PER_TRADE_PCT * MATURITY_MULTIPLIER["training"]
    assert decision["max_loss_dollars"] == pytest.approx(expected_risk, abs=0.01)
