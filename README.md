...
| **Tiered storage — passive history off Supabase Free** *(Phase 9, full design in [`docs/phase9-tiered-storage.md`](docs/phase9-tiered-storage.md))* | Closed paper trades, prices > 90 days, and 13F snapshots > 1 year live in passive storage (Hostinger 25 GB FTPS-archived JSONL.gz; optional Mac local sync). Active tables (open trades, recent events, agent_weights) stay in Supabase. Lets the system carry years of training data without ever paying Supabase Pro. Calibration cron reads BOTH active + archive when computing per-rule accuracy. | ~7h, ships incrementally |
