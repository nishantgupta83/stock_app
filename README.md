...
| **Real BUY/SELL graduation digest** | When a rule crosses the maturity gate (see `agents/_maturity.py`) and `is_mature` flips true, send a Telegram alert: "Rule X has graduated to BUY/SELL. Next signal containing this rule will fire as BUY/SELL instead of WATCH/AVOID_CHASE." Currently surfaces in the Calibration tab only. | 30min |
